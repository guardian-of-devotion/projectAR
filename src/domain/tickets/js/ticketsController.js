leantime.ticketsController = (function () {
                    ...
                    "#gantt", tasks, {
                        custom_popup_html: function (task) {
                            ...
                            if (task.type === 'milestone') {
                                ...
                                return '<div class="details-container"> ' +
                                    '<h4><a href="' + leantime.appUrl + '/tickets/showTicket/' + task.id + '" class="fa fa-map">' + htmlEntities(task.name) + '</a></h4><br /> ' +
                                    '<p>' + leantime.i18n.__("text.expected_to_finish_by") + ' <strong>' + end_date + '</strong><br /> ' +
                                    '' + Math.round(task.progress) + '%</p> ' +
                                    '<a href="' + leantime.appUrl + '/tickets/showTicket/' + task.id + '" class=""><span class="fa fa-map"></span> ' + leantime.i18n.__("links.edit_todo") + '</a> | ' +
                                    '<a href="' + leantime.appUrl + '/tickets/showKanban"><span class="iconfa-pushpin"></span> ' + leantime.i18n.__("links.view_todos") + '</a> ' +
                                    '<br><strong> plan hours ' + task.planHours + 'task_type ' + task.type + '</strong>' +
                                    '<br><strong> worked hours ' + task.workedHours +  '</strong>' +
                                    '</div>';
                            }
                        },
                        ...
                        on_date_change: function (task, start, end) {
                            if (task.type === 'milestone') {
                                ...
                                leantime.ticketsRepository.updateTicketDates(task.id, start, end);
                            }
                            _initModals();
    ...
    var initMarkerDropdown = function() {

        jQuery("body").on("click", ".markerDropdown .dropdown-menu a", function () {
            ...
            if (dataValue.length === 3) {
                ...
                var ticketId = dataValue[0];
                var markerId = dataValue[1];
                var markerVal = dataValue[2];
                ...
                jQuery.ajax(
                    ...
                    function () {
                        jQuery("#markerDropdownMenuLink" + ticketId + " span.text").text(markerVal);
                        jQuery.jGrowl(leantime.i18n.__("short_notifications.marker_updated"));

                    }
                );

            }else{
                console.log("Ticket Controller: Marker data value not set correctly");
            }
        });
    ...
    var initTicketSearchSubmit = function (url) {
        ...
        jQuery("#ticketSearch").on('submit', function(e) {
            ...
            var marker = jQuery("#markerSelect").val();
            ...
            if(marker != "" && marker != undefined) {query = query + "&marker=" + marker;}
    ...
    var initNoteKanban = function (noteStatusList) {
        ...
        jQuery(document).ready(function () {
            ...
            jQuery(".filterBar .row-fluid").css("opacity", "1");
            var height = jQuery("html").height() - 320;
            jQuery("#sortableNoteKanban .column .contentInner").css("height", height);

        });

        jQuery("#sortableNoteKanban .noteBox").hover(function () {
            jQuery(this).css("background", "#f9f9f9");
        }, function () {
            jQuery(this).css("background", "#ffffff");
        });
        ...
        jQuery("#sortableNoteKanban .contentInner").sortable({
            connectWith: ".contentInner",
            items: "> .moveable",
            tolerance: 'intersect',
            placeholder: "ui-state-highlight",
            ...
            stop: function (event, ui) {
                ...
                for (var i = 0; i < noteStatusList.length; i++) {

                    if (jQuery(".contentInner.status_" + noteStatusList[i]).length) {
                        statusPostData.payload[noteStatusList[i]] = jQuery(".contentInner.status_" + noteStatusList[i]).sortable('serialize');
                    }
                }
        ...
        function countNotes() {
            ...
            jQuery("#sortableNoteKanban .column").each(function () {
                var counting = jQuery(this).find('.moveable').length;
                jQuery(this).find(' .count').text(counting);
            });
    ...
    return {
        ...
        initMarkerDropdown:initMarkerDropdown,
