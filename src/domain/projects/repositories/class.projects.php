...
WHERE relation.userId = :id AND (project.active > '-1' OR project.active IS NULL)";
        ...
        /**
         * get activity percents
         *
         * @param $userId
         * @return array|false
         */
        public function getUserProjectsActivityPercent($userId)
        {
            $query = "
SELECT
    ...
    project.name,
    project.active,
    relation.activity_percent AS activityPercent,
    COUNT(ticket.projectId) AS ticketsAtUser
FROM zp_relationuserproject AS relation
         LEFT JOIN zp_projects as project ON project.id = relation.projectId
         LEFT JOIN zp_clients as client ON project.clientId = client.id
         LEFT JOIN zp_tickets as ticket ON project.id = ticket.projectId AND ticket.editorId = :id
WHERE relation.userId = :id AND (project.active > '-1' OR project.active IS NULL)
GROUP BY project.id, project.name, activityPercent
";
    ...
    public function getUserActivityPercentByProjectId($userId, $projectId)
    {
            $query = <<<SQL
SELECT
    relation.userId,
...
SQL;
        ...
        public function addProject($values)
        {
            ...
            $query = "INSERT INTO `zp_projects` (
				`name`, `details`, `clientId`, `hourBudget`, `dollarBudget`, `endAt`
			) VALUES (
				:name,
				:details,
				:clientId,
            ...
            $stmn->bindValue('endAt', $values['endAt'], PDO::PARAM_STR);
        ...
        public function insertProjectActivity($userId, $projectId, $projectActivity)
        {
            $sql = <<<SQL
UPDATE zp_relationuserproject
    SET activity_percent = :projectActivity
WHERE zp_relationuserproject.userId = :userId
  AND zp_relationuserproject.projectId = :projectId;
SQL;
            $stmn = $this->db->database->prepare($sql);

            $stmn->bindValue(':projectActivity', $projectActivity, PDO::PARAM_INT);
            $stmn->bindValue(':userId', $userId, PDO::PARAM_INT);
            $stmn->bindValue(':projectId', $projectId, PDO::PARAM_INT);
